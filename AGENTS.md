...
- **必须交两份**：`README.md`（中文）与 `README_EN.md`（英文），**两份都要在首屏放语言切换入口**，
  且互为镜像。只交一份 = 交付未完成；章节标题要翻译，顺序与事实（命令、数字、依赖版本、已知限制）必须逐项对齐，
  改一份就要同步另一份。
...
- 改完 README 跑 `npm run docs:check`（校验两份的锚点 + 双语一致性），不过不许提交。
